...
import { motion } from 'framer-motion'; // Used framer-motion since motion/react might not be installed, the user code is just a wrapper anyway.
...
export default function DecryptedText({
  ...
  const [displayText, setDisplayText] = useState(() => {
    if (animateOn === 'click' || animateOn === 'loop') {
      // Scramble initially to prevent flash of plain text
      const chars = useOriginalCharsOnly 
        ? Array.from(new Set(text.split(''))).filter((char: any) => char !== ' ')
        : characters.split('');
      return text.split('').map((char: any) => char === ' ' ? ' ' : chars[Math.floor(Math.random() * chars.length)]).join('');
    }
    return text;
  });
  ...
  /* Loop Behaviour */
  useEffect(() => {
    if (animateOn === 'loop' && !isAnimating && isDecrypted) {
      const timeout = setTimeout(() => {
        setRevealedIndices(new Set());
        setIsDecrypted(false);
        setDirection('forward');
        setIsAnimating(true);
      }, 3000); // 3-second pause at full text before restarting
      return () => clearTimeout(timeout);
    }
  }, [animateOn, isAnimating, isDecrypted]);
